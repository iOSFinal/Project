...
-(void)showwindow:(NSString*)info{
    ...
    hud.customView=[[UIImageView alloc]initWithImage:[self image:[UIImage imageNamed:@"chahao.png"] byScalingToSize:CGSizeMake(100, 100)]];
...
-(BOOL)checkinput{
    if(_username.text.length==0){
        /*UILabel *check=[[UILabel alloc]initWithFrame:CGRectMake(50, 380, 200, 30)];
        check.text=@"用户名不能为空";
        check.textColor=[UIColor redColor];
        [self.baseView addSubview:check];*/
        ...
        /*UILabel *check=[[UILabel alloc]initWithFrame:CGRectMake(50, 550, 200, 30)];
        check.text=@"重复密码错误";
        check.textColor=[UIColor redColor];
        [self.baseView addSubview:check];*/
...
-(void)signupOver{
    ...
    if([self checkinput]==true){
        ...
        [con setname:_username.text passwd:_passwd.text];
        controllers[0]=con;
